...
Download(){
	...
	new_ver_last="$(echo "${new_ver}" |cut -d"v" -f2)"
	wget --no-check-certificate -N "https://github.com/9seconds/mtg/releases/download/${new_ver}/mtg-${new_ver_last}-linux-${bit}.tar.gz"
	[[ ! -e "mtg-${new_ver_last}-linux-${bit}.tar.gz" ]] && echo -e "${Error} MTProxy 下载失败 !" && rm -rf "${file}" && exit 1
	tar -zxvf "mtg-${new_ver_last}-linux-${bit}.tar.gz"
	mv "mtg-${new_ver_last}-linux-${bit}" "mtg"
