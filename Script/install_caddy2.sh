...
install_caddy2(){
    if [ -z "$(command -v caddy)" ]; then
        if [[ "$os" =~ "Ubuntu" ]] || [[ "$os" =~ "Debian" ]]; then
            ...
            #修改caddy2监听端口为81和444
            sed -i '/:80/a\{' /etc/caddy/Caddyfile
            sed -i '/:80/a\    http_port 81' /etc/caddy/Caddyfile
            sed -i '/:80/a\    https_port 444' /etc/caddy/Caddyfile
            sed -i '/:80/a\}' /etc/caddy/Caddyfile
            sed -i '/:80/a' /etc/caddy/Caddyfile
            sed -i 's/:80/:81/g' /etc/caddy/Caddyfile
            #echo -e "" >>/etc/caddy/Caddyfile
            #echo -e "{" >>/etc/caddy/Caddyfile
            #echo -e "    http_port 81" >>/etc/caddy/Caddyfile
            #echo -e "    https_port 444" >>/etc/caddy/Caddyfile
            #echo -e "}" >>/etc/caddy/Caddyfile
        elif [[ "$os" =~ "CentOS" ]]; then
