...
install_caddy2(){
    if [ -z "$(command -v caddy)" ]; then
        if [[ "$os" =~ "Ubuntu" ]] || [[ "$os" =~ "Debian" ]]; then
            ...
            #如果已安装nginx修改caddy2监听端口为81和444，一便共存
            if [[ "$(command -v nginx)" ]]; then
                KeyTips="已安装“nginx”，为了避免监听端口冲突caddy需修改监听端口为81和444，下载已修改好的配置文件覆盖" && ShowColorTipsB
                run_comd="下载caddy配置文件" && success_info="成功" && failed_info="失败，请稍后手动下载"
                cd /etc/caddy/Caddyfile && curl -O https://raw.githubusercontent.com/vitaminx/OpenScript/main/Config/Caddyfile
                yes_or_no && ShowColorDottedLine && cd ~
            fi
        elif [[ "$os" =~ "CentOS" ]]; then
