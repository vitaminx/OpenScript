...
########################################################
    ...
    ShowColorPentagramTitle(){
        echo -e "\n${color_yellow}★★★★★ ${TitleiInformation} ★★★★★${color_end}\n"
    }
    # 输出彩色提示信息(必须定义变量)
    ShowColorTipsA(){
    ...
    os_identification(){
        ...
        if [ -f "/etc/redhat-release" ]; then
            if [[ $(cat /etc/redhat-release) =~ "CentOS" ]]; then
                os="CentOS" && TitleiInformation="您的操作系统为$os" && ShowColorPentagramTitle
            fi
        elif [[ "$osnamea" =~ "Ubuntu" ]] || [[ "$osnamec" =~ "Ubuntu" ]] || [[ "$osnameb" =~ "Ubuntu" ]]; then
            os="Ubuntu" && TitleiInformation="您的操作系统为$os" && ShowColorPentagramTitle
        elif [[ "$osnamea" =~ "Debian" ]] || [[ "$osnamec" =~ "Debian" ]] || [[ "$osnameb" =~ "Debian" ]]; then
            os="Debian" && TitleiInformation="您的操作系统为$os" && ShowColorPentagramTitle
